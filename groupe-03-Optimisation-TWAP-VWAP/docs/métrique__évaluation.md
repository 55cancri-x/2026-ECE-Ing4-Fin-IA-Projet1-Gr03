...
- **Optimisation sous contraintes (CSP / optimisation convexe)** (compromis entre impact de marché et tracking error)
...
## Impact de marché (proxy)

L’impact de marché est approximé par un **proxy quadratique normalisé par la liquidité** :
...
Cette métrique pénalise davantage une exécution agressive dans les tranches **peu liquides**, et pénalise moins lorsque le marché est **profond et liquide**.

## Erreur de tracking VWAP (L2)
...
où \(x_t^{VWAP}\) est la quantité cible à exécuter à la tranche \(t\), calculée à partir des volumes de marché par une répartition proportionnelle, puis **discrétisée** pour garantir que :

\[
\sum_{t=1}^{N} x_t^{VWAP} = Q
\]

## Objectif de l’optimisation sous contraintes (CSP)

La stratégie optimisée cherche un compromis entre impact de marché et qualité de suivi du VWAP, sous contraintes opérationnelles :

\[
\min_{x} \;\;
w_{\text{impact}} \sum_{t=1}^{N} \frac{x_t^2}{V_t + \varepsilon}
\;+\;
w_{\text{track}} \sum_{t=1}^{N} \left(x_t - x_t^{VWAP}\right)^2
\]

avec :
- \(x_t\) : quantité exécutée sur la tranche \(t\),
- \(V_t\) : volume de marché observé sur la tranche \(t\),
- \(x_t^{VWAP}\) : cible VWAP en volume,
- \(\varepsilon\) : constante numérique,
- \(w_{\text{impact}}\), \(w_{\text{track}}\) : poids de compromis entre impact et tracking.
...
\[
x_t \le \min(\alpha V_t,\; \overline{cap}_t)
\]

Ce mécanisme permet de modéliser :
- des fenêtres de news,
- des périodes de liquidité réduite,
- ou des règles opérationnelles spécifiques (no-trade partiel ou total).

## Analyse

Les résultats expérimentaux mettent en évidence les caractéristiques et limites de chaque approche :

- **TWAP** : simple à mettre en œuvre, mais rigide et insensible à la liquidité.
- **VWAP** : mieux aligné sur le marché, mais suppose une connaissance ou une estimation fiable des volumes futurs.
- **CSP** : fournit un compromis contrôlé entre impact et tracking, tout en respectant les contraintes de participation et de capacité par tranche.
- **RL** : approche adaptative capable de réagir dynamiquement aux variations du marché.
